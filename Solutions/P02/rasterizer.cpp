...
{   
    ...
    const Eigen::Vector2f A(_v[0].x(), _v[0].y());
    const Eigen::Vector2f B(_v[1].x(), _v[1].y());
    const Eigen::Vector2f C(_v[2].x(), _v[2].y());
    // Compute vectors
    auto v0 = C - A;
    auto v1 = B - A;
    auto v2 = P - A;
...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
    ...
    bbx[0] = std::round(std::min(v[0][0], std::min(v[1][0], v[2][0])));
    bbx[1] = std::round(std::max(v[0][0], std::max(v[1][0], v[2][0])));

    bby[0] = std::round(std::min(v[0][1], std::min(v[1][1], v[2][1])));
    ...
    for(int y = bby[0]; y <= bby[1]; y++ )
    {
        for(int x = bbx[0]; x <= bbx[1]; x++)
        {
            int count = 0;
            ...
            if(count!=0) {
                if (depth_buf[get_index(x, y)] > minDepth) {
                    ...
                    Eigen::Vector3f color = t.getColor() * count / offset.size();
                    set_pixel({x, y, minDepth}, color);
                }
            }
